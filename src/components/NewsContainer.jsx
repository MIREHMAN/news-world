...
function NewsContainer() {
  ...
  const url =
    "https://newsdata.io/api/1/latest?apikey=pub_58471ab3a2b0ae7bf04df3977d3c52e6814a6&country=us&prioritydomain=top";

  useEffect(() => {
    const getData = async () => {
      const response = await fetch(url);
      const data = await response.json();
      data === "" ? setLoading(true) : setArticles(data.results);
    };
    getData();
  }, []);
